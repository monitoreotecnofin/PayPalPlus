...
namespace Nop.Plugin.Payments.PayPalPlus.Components
{
    ...
    public class PaymentPayPalPlusViewComponent : NopViewComponent
    {
        ...
        {
            var processor = _paymentService.LoadPaymentMethodBySystemName("Payments.PayPalPlus") as PayPalPlusPaymentProcessor;
            if (processor == null ||
        ...
        protected string GetScriptPayment(Customer customer, AuthToken authToken, PayPalPaymentCreatedResponse createdPayment, int storeId)
        {
            ...
            scriptppp = scriptppp.Replace("{PayerFirstName}", customer.GetAttribute<string>(SystemCustomerAttributeNames.FirstName));
            scriptppp = scriptppp.Replace("{PayerLastName}", customer.GetAttribute<string>(SystemCustomerAttributeNames.LastName));
            scriptppp = scriptppp.Replace("{PayerPhone}", customer.GetAttribute<string>(SystemCustomerAttributeNames.Phone));
            ...
            scriptppp = scriptppp.Replace("{RCards}", customer.GetAttribute<string>("PPPTokenCards", _storeContext.CurrentStore.Id));            
