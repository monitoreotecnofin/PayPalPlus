    ...
    public class PayPalPlusPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        public override void Install()
        {
          ...
          $(document).on('accordion_section_opened', function (data) {
            ...
            string scriptPage = @"<script src='https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js'></script>
...
<script type='text/javascript'>  
        ...
        public ProcessPaymentResult ProcessPayment(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            var taxTotal = _orderTotalCalculationService.GetTaxTotal(shoppingCart, shippingRateComputationMethods, true);
            try
            {
                Task.Run(async () =>
                {
                    ...
                    try
                    {
                        PayPalPaymentExecutedResponse executedPayment = await ExecutePaypalPaymentAsync(http, authToken, createdPayment.id, payerId.ToString(), processPaymentRequest, payPalPlusPaymentSettings, urlipn, customer.ShoppingCartItems.ToList(), _orderTotalCalculationService, amountDetails, shippingRateComputationMethods, taxTotal);
        ...
        private static async Task<PayPalPaymentExecutedResponse> ExecutePaypalPaymentAsync(HttpClient http, AuthToken accessToken, string paymentId, string payerId, ProcessPaymentRequest processPaymentRequest, PayPalPlusPaymentSettings paypalPlusPaymentSettings, string urlnofications, IList<ShoppingCartItem> cart, IOrderTotalCalculationService orderTotalCalculationService, DetailsAmountInfo amountInfo, IList<IShippingRateComputationMethod> shippingRateComputationMethods, decimal taxTotal)
        {
            ...
            //get tax total
            //var taxTotal = _orderTotalCalculationService.GetTaxTotal(cart, shippingRateComputationMethods, true);
            var shipping = orderTotalCalculationService.GetShoppingCartShippingTotal(cart, false,shippingRateComputationMethods);
            var shippingTotal = shipping ?? 0;
